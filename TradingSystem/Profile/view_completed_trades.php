...
<body>
        ...
        <div class="white-rectangle" style="position: fixed; top: 20px; z-index: 1;">
            ...
            <div class="search-bar">
                ...
                <div class="search-box">
                    <input type="text" id="search-input" placeholder="I miei scambi completati" onkeyup="enablesearch()">
    ...
    <div class="card-container" style="position: absolute; top: 30px">
        <?php include 'get_completed_trades.php'; ?>
            <?php if(isset($performedquery) && !empty($annunci)): ?>
                <?php foreach ($annunci as $coppia_annunci) : ?> 
                    <div class="double-card-container">
                        <div class="double-card">
                            <h2 class="double-card-title">SCAMBIO CON <?php if($coppia_annunci[0][4] === $_SESSION["username"]){echo $coppia_annunci[1][4];}else echo $coppia_annunci[0][4]; ?>, IN DATA: <?php echo $coppia_annunci[2]; ?></h2>
                            <h2 class="double-card-subtitle">Contatta <?php if($coppia_annunci[0][4] === $_SESSION["username"]){echo $coppia_annunci[1][4];}else echo $coppia_annunci[0][4]; ?> all'indirizzo: <?php if($coppia_annunci[0][4] === $_SESSION["username"]){echo $coppia_annunci[1][5];}else echo $coppia_annunci[0][5]; ?></h4>
                            <div class="card-wrapper">
                            <div class="card">
                                <img src="../<?php echo $coppia_annunci[0][1]; ?>">
                                <div class="card-content">
                                    <h3><?php echo $coppia_annunci[0][0]; ?></h3>
                                    <p>In <?php echo $coppia_annunci[0][3]; ?></p>
                                    <p>Di <a href="../Profile/view_reviews.php?username=<?=$coppia_annunci[0][4];?>" style="color: #FF9F00"><?php echo $coppia_annunci[0][4]; ?></a></p>
                                <a href="../LoggedIn/view_insertion.php?ID=<?= $coppia_annunci[0][2] ?>" class="btn">Visualizza</a>
                                </div>
                            </div>
                            <img src="img/arrows.png" clas="rounded-circle" width="150" height="150" style="margin-top: 150px">
                            <div class="card" style="margin-left: 30px;">
                                <img src="../<?php echo $coppia_annunci[1][1]; ?>">
                                <div class="card-content">
                                    <h3><?php echo $coppia_annunci[1][0]; ?></h3>
                                    <p>In <?php echo $coppia_annunci[1][3]; ?></p>
                                    <p>Di <a href="../Profile/view_reviews.php?username=<?=$coppia_annunci[1][4];?>" style="color: #FF9F00"><?php echo $coppia_annunci[1][4]; ?></a></p>
                                <a href="../LoggedIn/view_insertion.php?ID=<?= $coppia_annunci[1][2] ?>" class="btn">Visualizza</a>
                                </div>
                            </div>
                            </div>
                        </div>
                        
                        <?php if($coppia_annunci[3] !== true) {?>
                            <div class="button-container">
                            <?php 
                                $altroUtente = "";
                                if($coppia_annunci[0][4] === $_SESSION["username"]) $altroUtente = $coppia_annunci[1][4];
                                else $altroUtente = $coppia_annunci[0][4];
                                $link = "LeaveReview.php?utente_scrivente=" . $_SESSION["username"] . "&utente_ricevente=" . $altroUtente . "&annuncio_avvio=" . $coppia_annunci[0][2] . "&annuncio_completamento=" . $coppia_annunci[1][2];
                            ?>
                                <a href="<?= $link ?>" class="btn" style="margin-left: 200px">Lascia una recensione a <?php echo $altroUtente ?></a>
                            </div>
                        <?php
                        }else{?>
                            <div class="button-container"><p class="dis-btn" style="margin-left: 200px;">Recensione inviata</p></div>
                        <?php
                        }
                        ?>
